...
public class PlayerH2Service implements PlayerRepository {
    ...
    public Player updatePlayer(int playerId, Player player) {
        if (player.getPlayerName() != null) {
            db.update("update from team set playerName=? where playerId=?", player.getPlayerName(), playerId);
        }
        if (player.getJerseyNumber() != 0) {
            db.update("update from team set jerseyNumber=? where playerId=?", player.getJerseyNumber(), playerId);
        }
        if (player.getRole() != null) {
            db.update("update from team set role=? where playerId=?", player.getRole(), playerId);
        }
